...
bool smp_group_img_mgmt::extract_hash(QByteArray *file_data, QByteArray *hash)
{
    ...
    {
        std::cerr << "Decoded image size is not valid: " << img_size;
        return false;
    }
        ...
        while (new_pos < pos + tlv_area_length)
        {
            ...
            if (type == image_tlv_tag_sha256)
            {
                ...
                else
                {
                    ...
                    std::cerr << "Invalid length hash found";
                }
            }
...
//            STATUS_UNSUPPORTED
    ...
    else if (command == COMMAND_UPLOAD && mode == MODE_UPLOAD_FIRMWARE)
    {
        ...
        // TODO - fix this because it is constantly crashing:
