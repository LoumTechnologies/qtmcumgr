...
void smp_processor::message_received(smp_message *response)
{
    ...
    {
        std::cerr << "Invalid group, expected " << last_message_header->nh_group << " got " << response_header->nh_group;
    }
    else if (response_header->nh_id != last_message_header->nh_id)
    {
        std::cerr << "Invalid command, expected " << last_message_header->nh_id << " got " << response_header->nh_id;
    }
    else if (response_header->nh_seq != last_message_header->nh_seq)
    {
        std::cerr << "Invalid sequence, expected " << last_message_header->nh_seq << " got " << response_header->nh_seq;
    }
    else if (response_header->nh_op != smp_message::response_op(last_message_header->nh_op))
    {
        std::cerr << "Invalid op, expected " << smp_message::response_op(last_message_header->nh_op) << " got " << response_header->nh_op;
    }
    else
    {
        //Headers look valid
        ...
        if (i == group_handlers.length())
        {
            ...
            std::cerr << "No registered handler for group " << group << ", dropping response.";
            this->cleanup();
            return;
        }
...
bool smp_processor::decode_message(QCborStreamReader &reader, uint8_t version, uint16_t level, QString *parent, smp_error_t *error)
{
    ...
    if (reader.lastError())
    {
        std::cerr << "Failed to parse CBOR message: " << reader.lastError().toString().toStdString();
        return false;
    }
